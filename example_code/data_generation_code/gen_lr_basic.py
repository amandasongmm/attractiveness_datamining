...
def linear_model_eval_metrics(y_gt, y_predict):
    ...
    # Model evaluations: correlation, mean absolute error
    rater_num = y_gt.shape[1]  # y_gt shape: num_data_point * num_raters
    corr_arr, mae_arr = [np.zeros((rater_num, 1))] * 2

    for i in range(rater_num):

        # corr_arr[i] = np.corrcoef(y_gt[:, i], y_predict[:, i])[0, 1]
        corr_arr[i], _ = pearsonr(y_gt[:, i], y_predict[:, i])
        mae_arr[i] = mean_absolute_error(y_gt[:, i], y_predict[:, i])
        a = 12

        if corr_arr[i] > 1:
            print 'error', str(corr_arr)
    [corr_arr, mae_arr] = [x.flatten() for x in [corr_arr, mae_arr]]

    return corr_arr, mae_arr
...
def plain_linear_regression(orig_feature_arr, full_rating, p):  # feature_arr: 200*7, full_rating: 200* 1540
    ...
    orig_feature_arr = preprocessing.scale(orig_feature_arr)
    ...
    corr_train, mae_train, corr_test, mae_test = [np.zeros((rater_num, p.k_fold))] * 4
    ...
    for i, (train_ind, test_ind) in enumerate(kf):
        ...
        corr_train[:, i], mae_train[:, i] = linear_model_eval_metrics(y_train, y_train_predict)
        corr_test[:, i], mae_test[:, i] = linear_model_eval_metrics(y_test, y_test_predict)

    # ensemble model coefficient
    coef_back_all = coef_back_all/p.k_fold
    print corr_train
    # training and test metrics
    [corr_train, mae_train, corr_test, mae_test] = \
        [np.nanmean(x, axis=1) for x in [corr_train, mae_train, corr_test, mae_test]]

    np.savez('../tmp/LR_model_params', coef=coef_back_all, corr_train=corr_train, corr_test=corr_test,
